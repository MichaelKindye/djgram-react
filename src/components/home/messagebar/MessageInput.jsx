...
const MessageInput = ({handleKeyDown, setInputValue, handleMessageSending}) => {
    return(
        <Card className="border-none bg-gray-900 z-50 p-0 w-full flex gap-0 flex-row items-center rounded-none">
            <div className='w-[95%]'>
               <div onKeyDown={(event) => handleKeyDown(event)} onInput={(e) => {setInputValue(e.target.innerText);}} contentEditable="true" className='[scrollbar-width:none] [-ms-overflow-style:none] [&::webkit-scrollbar]:hidden w-full py-4 px-2 leading-5 outline-none text-white break-words whitespace-pre-wrap overflow-y-auto max-h-[12rem]'></div>
            </div>
